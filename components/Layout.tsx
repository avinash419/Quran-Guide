...
const Layout: React.FC<LayoutProps> = ({ children, currentSection, setSection }) => {
  return (
    <div className="flex flex-col min-h-screen max-w-2xl mx-auto bg-white shadow-2xl shadow-stone-200/50 relative">
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md bg-emerald-950/90 backdrop-blur-2xl rounded-3xl border border-white/10 flex justify-around p-3 z-50 shadow-2xl shadow-emerald-900/40">
        <NavButton 
...
const NavButton: React.FC<{ active: boolean; label: string; icon: React.ReactNode; onClick: () => void }> = ({ active, label, icon, onClick }) => (
  <button 
    onClick={onClick}
    className={`flex flex-col items-center gap-1.5 transition-all duration-500 px-5 py-2 rounded-2xl ${active ? 'bg-white/10 text-white scale-105' : 'text-emerald-200/40 hover:text-emerald-100'}`}
  >
    <span className={`w-5 h-5 transition-transform duration-300 ${active ? 'scale-110' : ''}`}>{icon}</span>
    <span className={`text-[10px] font-bold tracking-widest uppercase ${active ? 'opacity-100' : 'opacity-60'}`}>
      {label}
    </span>
  </button>
);
